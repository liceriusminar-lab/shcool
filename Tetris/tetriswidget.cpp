...
TetrisWidget::TetrisWidget(QWidget *parent) : QWidget(parent)
{
    ...
    connect(flashTimer, &QTimer::timeout, this, [this]() {
        flashStep++;
        if (flashStep >= 6) {
            // Анимация закончена — удаляем линии
            flashTimer->stop();
            flashStep = 0;
...
void TetrisWidget::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_Down:
        ...
        return;
    case Qt::Key_Up:
        // Wall kick: пробуем повернуть, если не получается — сдвигаем
    ...
    case Qt::Key_Shift:
        ...
        if (canHold) {
            ...
            if (heldTetromino == nullptr) {
                ...
                Tetromino *tmp   = heldTetromino;
                heldTetromino    = currentTetromino;
                currentTetromino = tmp;
            }
            // Сбрасываем позицию held фигуры
            heldTetromino = new Tetromino(heldTetromino->getType());
            // Пересоздаём чтобы сбросить position и rotation
            Tetromino::Type t = currentTetromino->getType();
            delete currentTetromino;
            currentTetromino = new Tetromino(t);
        }
        break;
    default:
...
int TetrisWidget::ghostDropY() const
{
    ...
    QPoint firstBlock = ghost.getBlocks().first();
    QPoint firstCur   = currentTetromino->getBlocks().first();
    return firstBlock.y() - firstCur.y();
}
...
{
    // Собираем полные линии (не удаляем сразу — ждём анимацию)
    flashLines.clear();
    ...
    if (!flashLines.isEmpty()) {
        ...
        // Запускаем анимацию — спавн произойдёт после неё
        gameTimer->stop();
        flashStep = 0;
        flashTimer->start();
    } else {
...
void TetrisWidget::spawnNewTetromino()
{
    ...
    if (!isValidPosition(*currentTetromino)) {
        ...
        QMessageBox::information(this, "Игра окончена",
                                 QString("Ваш счёт: %1").arg(score));
    }
}

// Рисует один блок с градиентом
...
void TetrisWidget::paintEvent(QPaintEvent *)
{
    ...
    bgGrad.setColorAt(0.0, QColor(15, 15, 30));
    bgGrad.setColorAt(1.0, QColor(5,  5,  15));
    p.fillRect(rect(), bgGrad);

    static const QVector<QColor> colors = {
        QColor(0,   220, 220), // I  cyan
        QColor(220, 200,   0), // O  yellow
        QColor(180,   0, 200), // T  magenta
        QColor(0,   200,   0), // S  green
        QColor(220,   0,   0), // Z  red
        QColor(0,    80, 220), // J  blue
        QColor(220, 120,   0)  // L  orange
    };
